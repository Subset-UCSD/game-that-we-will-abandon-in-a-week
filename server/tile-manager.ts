	...
	return (
		map
			...
			.map(([key, { tileChars, tiles }]) => {
				...
				return `${key}\n${tileChars
					.entries()
					.map(([tile, char]) => `${char} ${tile}`)
					.toArray()
					.sort()
					.join("\n")}\n${tiles
					.map((tile, i) => (i > 0 && i % CHUNK_SIZE === 0 ? "\n" : "") + (tile === null ? " " : tileChars.get(tile)))
					.join("")}.`;
			})
			.toArray()
			.sort()
	...
	return new Map(
		t.split(/\r?\n\r?\n/).map((chunk) => {
			...
			const tiles = lines
				.slice(-CHUNK_SIZE)
				.values()
				.flatMap((line) => line.slice(0, CHUNK_SIZE).padEnd(CHUNK_SIZE)[Symbol.iterator]())
				.map((char) => (char === " " ? null : (revCharMap.get(char) ?? null)))
				.toArray();
			if (tiles.length !== CHUNK_SIZE * CHUNK_SIZE) {
