	...
	await Promise.all([
		fetch("./assets/what-do-sheep-become-when-they-die1.png")
			...
			.then(
				(image): Entry => [
					"cloud",
					{ image, lore: ["CLOUD 雲", "when sheep die", "process known as", "condensation"] },
				],
			),
		fetch("./assets/dager.png")
			...
			.then(
				(image): Entry => [
					"knife",
					{ image, lore: ["KNIFE 刀", "convenient to use", "was $0.97 on temu", "hold K to use"] },
				],
			),
		fetch("./assets/meatball1.png")
			...
			.then(
				(image): Entry => [
					"meatball",
					{ image, lore: ["MEAT BALL", "scrumptious", "off brand ikea ball", "these ones spicy"] },
				],
			),
...
export function renderInventory({ c }: Canvas, items: Player["items"]): void {
	...
	for (const [i, { item, count }] of items.entries()) {
		...
		if (entry) {
			...
			c.drawImage(image, x - IMG_EXPAND, GAP - IMG_EXPAND, ITEM_SIZE + 2 * IMG_EXPAND, ITEM_SIZE + 2 * IMG_EXPAND);
